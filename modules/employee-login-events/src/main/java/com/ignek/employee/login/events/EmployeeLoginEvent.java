...
public class EmployeeLoginEvent implements LifecycleAction {
	...
	public void processLifecycleEvent(LifecycleEvent lifecycleEvent) throws ActionException {
		try {
			...
			String emailAddress = portal.getUser(request).getEmailAddress();
			...
			if (Validator.isNotNull(objectDefinition)) {
				...
				values.put(EmployeeLoginConstants.DETAILS, emailAddress);
				values.put(EmployeeLoginConstants.IP_ADDRESS, ipAddress);
				values.put(EmployeeLoginConstants.ACTIVITY_USER_ID, userId);

				ObjectEntry objectEntry = objectEntryLocalService.addObjectEntry(portal.getUser(request).getUserId(), groupId, objectDefinition.getObjectDefinitionId(), values, serviceContext);
				log.info("ObjectEntry created for login event : " + objectEntry.getObjectEntryId());
			}
		} catch (Exception e) {
